...
if [ "${CM_SKIP_BUILD}" != "yes" ] ; then
 ...
 make SHARED_LIBRARY=true PREFIX=${CM_INSTALL_DIR} MANPREFIX=${CM_INSTALL_DIR} LIKWIDFILTERPATH=${CM_INSTALL_DIR}/bin
  if [ "${?}" != "0" ] ; then
    echo "Error: Compilation failed at $PWD!" 
    exit 1
  fi
 make install SHARED_LIBRARY=true PREFIX=${CM_INSTALL_DIR} MANPREFIX=${CM_INSTALL_DIR} LIKWIDFILTERPATH=${CM_INSTALL_DIR}/bin
  if [ "${?}" != "0" ] ; then
    echo "Error: Compilation failed at $PWD!" 
    exit 1
  fi
 ...
 make CFLAG=-fPIC PREFIX=${CM_INSTALL_DIR} MANPREFIX=${CM_INSTALL_DIR} LIKWIDFILTERPATH=${CM_INSTALL_DIR}/bin
  if [ "${?}" != "0" ] ; then
    echo "Error: Compilation failed at $PWD!" 
    exit 1
  fi
 make install CFLAG=-fPIC PREFIX=${CM_INSTALL_DIR} MANPREFIX=${CM_INSTALL_DIR} LIKWIDFILTERPATH=${CM_INSTALL_DIR}/bin
  if [ "${?}" != "0" ] ; then
    echo "Error: Compilation failed at $PWD!" 
    exit 1
  fi
