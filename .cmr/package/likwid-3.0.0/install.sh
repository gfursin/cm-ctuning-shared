...
if [ "${CM_SKIP_BUILD}" != "yes" ] ; then
 ...
 make PREFIX=${CM_INSTALL_DIR} MANPREFIX=${CM_INSTALL_DIR} LIKWIDFILTERPATH=${CM_INSTALL_DIR}/bin
  if [ "${?}" != "0" ] ; then
    echo "Error: Compilation failed at $PWD!" 
    exit 1
  fi
 make install PREFIX=${CM_INSTALL_DIR} MANPREFIX=${CM_INSTALL_DIR} LIKWIDFILTERPATH=${CM_INSTALL_DIR}/bin
  if [ "${?}" != "0" ] ; then
    echo "Error: Compilation failed at $PWD!" 
    exit 1
  fi
